...
//       qDebug()<<"graph2 added"<<endl;
        ...
        ui->widget->graph(i+function1_count)->setPen(pen);
...
//            qDebug()<<'('<<x1[j]<<','<<y1[i][j]<<')'<<endl;
    ...
    //设置数据
    for(int j=0;j<function2_count;j++){
        ui->widget->graph(j+function1_count)->setData(x1[j],y1[j]);
        ui->widget->graph(j+function1_count)->rescaleAxes(true);
    }

//    qDebug()<<"line1 ready"<<endl;
//画第二段
    QVector<QVector<double>> y2(5);
...
//       qDebug()<<"graph3 added"<<endl;
        ...
        ui->widget->graph(i+function1_count+function2_count)->setPen(pen);
...
//            qDebug()<<'('<<x2[j]<<','<<y2[i][j]<<')'<<endl;
        }
    }
    //设置数据
    for(int j=0;j<function2_count;j++){
        ui->widget->graph(j+function1_count+function2_count)->setData(x2[j],y2[j]);
        ui->widget->graph(j+function1_count+function2_count)->rescaleAxes(true);
    }
}
